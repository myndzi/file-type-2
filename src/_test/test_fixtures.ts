...
export const getFixtures = async (ext: string): Promise<string[]> => {
  const arr = Object.prototype.hasOwnProperty.call(names, ext)
    ? (names as Record<string, string[]>)[ext].map((name: string) => `${name}.${ext}`)
    : [`fixture.${ext}`];

  return (
    await Promise.all(
      arr.map(fn => {
        const path = resolve(import.meta.dirname, 'fixture', fn);
...
const names = {
  ...
  // mif1 / msf1 should be heif: https://nokiatech.github.io/heif/technical.html#:~:text=File%20extension-,mif1,.heif,-heic
  heif: [
    'fixture-mif1',
    'fixture-msf1',
  ],
